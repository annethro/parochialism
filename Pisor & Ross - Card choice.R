...
setwd("C:/Users/anne.pisor/OneDrive - Washington State University (email.wsu.edu)/Research/Parochialism/")
...
these[which(colnames(full1) %in% row.names(which.mat)[rowSums(which.mat) > 0])]<-"pmm"
full2 <- mice(full1, method = these, predictorMatrix = which.mat, seed = semilla, m = 10)
...
priors <- c(set_prior("normal(0,1)", class = "b"),
            ...
            set_prior("exponential(1)", class = "sd")) # Set weakly informative priors on the relevant scale of the parameters. See https://xcelab.net/rm/statistical-rethinking/ for a discussion of choice of priors.
...
part <- part[!is.na(part$Average_Given.Out),] # Not everyone was presented with an out-group in the 2014-15 economic game. See Supplement for details.
...
priors <- c(set_prior("normal(0,1)", class = "b"),
                set_prior("normal(0,1)", class = "Intercept"),
                set_prior("exponential(1)", class = "sd")) # Set weakly informative priors. See https://xcelab.net/rm/statistical-rethinking/ for a discussion of choice of priors.
...
df_c14_1$Variable[df_c14_1$Variable %in% "Loc.SameValley_DiffM1"] <- "Alter lives in a nearby community"
...
df_c14_1$Variable[df_c14_1$Variable %in% "Pueblo.Other_DiffM1"] <- "Alter is from other pueblo indígena"
